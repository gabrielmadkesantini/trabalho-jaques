<div style="text-align: center; margin-top: 5%">
    ...
    <form action="{{ url()->current() }}" method="POST" enctype="multipart/form-data">
        @csrf
        <input type="text" name="nome" placeholder="Titulo do filme"
            value="{{ old('nome', $movie->nome ?? '') }}"><br>
        <textarea name="sinopse" cols="40" rows="5" placeholder="Sinopse do filme">
            {{ old('sinopse', $movie->sinopse ?? '') }}
        </textarea>
        ...
        <fieldset style="margin-left: 40%; margin-right: 40%">
            ...
            <input type="number" min="1899" max="2023" name="ano" value="{{ old('ano', $movie->ano ?? '') }}"> <br>
        </fieldset>
        ...
        <select name="categorias[]" multiple>
            @foreach ($categorias as $categoria)
                <option value="{{ $categoria->id }}" @if (in_array($categoria->id, $movie->categorias->pluck('id')->toArray())) selected @endif>
                    {{ $categoria->name }}
                </option>
            @endforeach
        </select>
